...
from keras.backend import cast, argmax
...
def EEGTCNet(nb_classes,Chans=64, Samples=128, layers=3, kernel_s=10,filt=10, dropout=0, activation='relu', F1=4, D=2, kernLength=64, dropout_eeg=0.1):
    ...
    dense        = Dense(output_dim=nb_classes, name = 'dense',kernel_constraint = max_norm(regRate))(out)
    softmax      = Activation('softmax', name = 'softmax')(dense)
    if False:
        print("TCNET infos: ")
        print("input1: {}".format(input1))
        print("input2: {}".format(input2))
        print("block2 eegtcnetshape: {}".format(block2.shape))
